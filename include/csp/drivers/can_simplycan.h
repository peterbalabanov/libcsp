#ifndef LIB_CSP_INCLUDE_CSP_DRIVERS_CAN_SIMPLYCAN_H_
#define LIB_CSP_INCLUDE_CSP_DRIVERS_CAN_SIMPLYCAN_H_

/**
   @file

   simplyCAN driver (Windows).

   This driver requires the simplycan library.
*/
...
/**
   Open serial port with CAN adapter and add CSP interface.

   @param[in] device CAN device name (Windows device).
...
*/
int csp_can_simplycan_open_and_add_interface(const char * device, const char * ifname, int bitrate, bool promisc, csp_iface_t ** return_iface);

/**
   Initialize simplycan and add CSP interface.

   @deprecated version 1.6, use csp_can_simplycan_open_and_add_interface()

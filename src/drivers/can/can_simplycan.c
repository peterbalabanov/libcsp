...
static int csp_can_tx_frame(void *driver_data, uint32_t id, const uint8_t *data, uint8_t dlc)
{
    ...
    can_sts_t canstatus = {0};
    
    if (!simply_send(&can_msg))
    {
        simply_can_status(&canstatus);
        csp_log_error("%s[%s]: simplycan error in tx: %d, simplycan status: %x, free tx elements: %d", __FUNCTION__, ctx->name,
            simply_get_last_error(), canstatus.sts, canstatus.tx_free);
            return CSP_ERR_TX;
        }
    ...
    while (canstatus.sts && CAN_STATUS_PENDING)
    {
        ...
        {
            csp_log_error("%s[%s]: simplycan error in status petition: %d", __FUNCTION__, ctx->name, simply_get_last_error());
            return CSP_ERR_TX;
        }
        if (elapsed_ms >= 1000)
        {
            csp_log_warn("%s[%s]: tx frame check timed out", __FUNCTION__, ctx->name);
                                             ...
                                             csp_iface_t **return_iface)
{
    ...
    bool result = true;
    identification_t simplycanID = {0};
    result &= simply_open((char *)device);
    result &= simply_identify(&simplycanID);
    result &= simply_initialize_can(bitrate);
    result &= simply_start_can();

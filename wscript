...
def configure(ctx):
    ...
    if ctx.options.enable_can_simplycan:
        ...
        #ctx.check_cfg(package='libsimplycan', args='--cflags --libs', define_name='CSP_HAVE_LIBSIMPLYCAN')
        ctx.env.append_unique('LIBS', 'simplyCAN-64')
        ctx.env.append_unique('LIBDIR', 'C:\\usr\\Work_dir\\Programming\\Repos\\UPMSat3Tools\\lib\\libcsp')
